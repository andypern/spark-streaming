...
object m7import {
  def main(args: Array[String]) {
    if (args.length < 5) {
      System.err.println("Usage: m7import <master> <hostname> <port> <batchsecs> <tablename>\n" +
    ...
    if(!admin.isTableAvailable("/tables/" + tablename )) {
        println("Table doesn't exist..quitting")
        System.exit(1)
        //admin.createTable(tableDesc)
      }

      val table = new HTable(conf, "/tables/"+ tablename)
      ...
      val rddarray = rdd.collect
        if(rddarray.length > 0) {
          for(line <- rddarray) {
            ...
            val tblPut = new Put(Bytes.toBytes(myKey))
            //build our tblPut object with multiple columns.
            // TODO: probably better done w/ a loop, but thta's for another day.

            tblPut.add(Bytes.toBytes("cf1"),Bytes.toBytes("resID"),Bytes.toBytes(resID))
            tblPut.add(Bytes.toBytes("cf1"),Bytes.toBytes("date"),Bytes.toBytes(date))
            tblPut.add(Bytes.toBytes("cf1"),Bytes.toBytes("time"),Bytes.toBytes(time))
